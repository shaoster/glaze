...
- **Docker Compose**: A completely self-contained deployment setup that includes PostgreSQL, Redis, and Nginx with Let's Encrypt SSL. Perfect for deploying locally or to a standard virtual machine (VM). See the [Docker Compose Deployment Guide](infra/custom/README.md) for step-by-step instructions.
