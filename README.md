...
## Deployment
Glaze is configured to deploy on [Render.com](https://render.com/) with a Django backend and static Vite frontend. The deployment uses [Render's Blueprint workflow](https://render.com/docs/infrastructure-as-code) via [`render.yaml`](render.yaml) for infrastructure-as-code deployment.

### Blueprint deployment (render.yaml)
The [`render.yaml`](render.yaml) file defines the complete deployment configuration:
- **Database:** Render PostgreSQL (free plan) named `glaze-prod`
- **Web service:** Python runtime with Gunicorn + Uvicorn
- **Build command:** Runs [`build.sh`](build.sh) to build the React bundle and collect static files
- **Start command:** Runs `python -m gunicorn backend.asgi:application -k uvicorn.workers.UvicornWorker`
...
4. Select your GitHub repository and click "Connect"
5. Give the blueprint a name and click "Apply"

Render will automatically provision the database and web service with all configured settings.

### Environment variables
The following environment variables are automatically configured or can be added in the `render.yaml`:

| Variable | Source/Purpose |
|---|---|
| `DATABASE_URL` | Auto-populated from PostgreSQL database connection |
| `SECRET_KEY` | Auto-generated by Render |
| `WEB_CONCURRENCY` | Set to 4 for optimal worker performance |
| `GOOGLE_OAUTH_CLIENT_ID` | Add via Render Dashboard to enable Google Sign-In |
| `CLOUDINARY_CLOUD_NAME` | Add via Render Dashboard if using image uploads |
| `CLOUDINARY_API_KEY` | Add via Render Dashboard if using image uploads |
| `CLOUDINARY_API_SECRET` | Add via Render Dashboard if using image uploads |
...
Render automatically runs this script during each deploy. The Vite-built frontend is served at `/` while Django API endpoints remain available at `/api/`.

### Configuration details
- **Database:** PostgreSQL is configured via `DATABASE_URL` and `dj_database_url` in [`backend/settings.py`](backend/settings.py). Connection is automatic when deployed via `render.yaml`.
...
- **For more details:** See [Render's Django deployment guide](https://docs.render.com/deploy-django) and [Render's Blueprint specification](https://render.com/docs/blueprint-spec).
