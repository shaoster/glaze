...
from api.models import AsyncTask, CropRun, Image, Piece, PieceState


def _make_mask_b64(has_subject: bool = True) -> str:
    """Create a minimal valid RGBA PNG mask, optionally with non-zero alpha."""
...
def piece_with_image(db, user, image):
    ...
    PieceState.objects.create(piece=p, state="designed", order=1)
    return p
...
class TestRunCropInference:
    def test_crop_run_on_success(self, image):
        ...
        with patch(
            ...
            with patch("api.utils.upload_mask_to_cloudinary", return_value=None):
                crop_run = run_crop_inference(image)
        ...
        assert crop_run.image_id == image.id

    def test_crop_run_on_no_subject(self, image):
        from api.utils import run_crop_inference
    ...
    def test_crop_run_on_empty_alpha_mask(self, image):
        from api.utils import run_crop_inference
    ...
    def test_crop_run_on_error(self, image):
        from api.utils import run_crop_inference
    ...
    def test_crop_run_associates_async_task(self, image, user):
        from api.utils import run_crop_inference
...
class TestCropRunViewSet:
    ...
    def test_human_run_owner_allowed(self, user, piece_with_image, image):
    ...
    def test_human_run_admin_allowed(self, db, image):
...
class TestImageCropRunsView:
    ...
    def test_non_owner_gets_empty(self, other_user, image):
        CropRun.objects.create(
            image=image,
            source={
                "type": "automated",
