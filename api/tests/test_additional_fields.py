...
def _make_piece_with_state(
    state_id: str, custom_fields: dict | None = None
) -> PieceState:
...
class TestAdditionalFieldsValidation:
    ...
    def test_optional_string_field_accepts_string(self, db):
        with (
            ...
            ps = _make_piece_with_state(
                "mock_typed", {"required_num": 1, "optional_str": "hello"}
            )
            ps.save()  # must not raise
    ...
    def test_optional_string_field_wrong_type_fails(self, db):
        with (
            ...
            ps = _make_piece_with_state(
                "mock_typed", {"required_num": 1, "optional_str": 99}
            )
            with pytest.raises(ValueError, match="custom_fields validation failed"):
                ps.save()
    ...
    def test_integer_field_accepts_integer(self, db):
        with (
            ...
            ps = _make_piece_with_state(
                "mock_typed", {"required_num": 1, "int_field": 7}
            )
            ps.save()  # must not raise
    ...
    def test_integer_field_rejects_float(self, db):
        with (
            ...
            ps = _make_piece_with_state(
                "mock_typed", {"required_num": 1, "int_field": 7.5}
            )
            with pytest.raises(ValueError, match="custom_fields validation failed"):
                ps.save()
    ...
    def test_boolean_field_accepts_bool(self, db):
        with (
            ...
            ps = _make_piece_with_state(
                "mock_typed", {"required_num": 1, "bool_field": True}
            )
            ps.save()  # must not raise
    ...
    def test_enum_field_valid_value_passes(self, db):
        with (
            ...
            ps = _make_piece_with_state(
                "mock_typed", {"required_num": 1, "enum_field": "beta"}
            )
            ps.save()  # must not raise
    ...
    def test_enum_field_invalid_value_fails(self, db):
        with (
            ...
            ps = _make_piece_with_state(
                "mock_typed", {"required_num": 1, "enum_field": "delta"}
            )
            with pytest.raises(ValueError, match="custom_fields validation failed"):
                ps.save()
    ...
    def test_extra_key_fails(self, db):
        with (
            ...
            ps = _make_piece_with_state(
                "mock_typed", {"required_num": 1, "undeclared": "x"}
            )
            with pytest.raises(ValueError, match="custom_fields validation failed"):
                ps.save()
    ...
    def test_state_ref_wrong_type_fails(self, db):
        with (
            ...
            ps = _make_piece_with_state(
                "mock_with_ref", {"carried_num": "not-a-number"}
            )
            with pytest.raises(ValueError, match="custom_fields validation failed"):
                ps.save()
