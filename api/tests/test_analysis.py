...
def _attach_combo(
    ...
    field_name: str = "glaze_combination",
) -> None:
...
class TestQualifyingStateFilter:
    ...
    def test_images_from_non_qualifying_state_excluded(self, client, user, combo):
        ...
        PieceState.objects.create(
            user=user, piece=p, state="wheel_thrown", images=[SAMPLE_IMAGE], notes=""
        )
        response = client.get(URL)
        data = response.json()
    ...
    def test_images_from_multiple_qualifying_states_aggregated(
        self, client, user, combo
    ):
        p = _piece(user)
        glazed = _add_state(p, "glazed", images=[SAMPLE_IMAGE])
        _attach_combo(glazed, combo)
