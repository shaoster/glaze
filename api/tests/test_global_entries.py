...
class TestAutoGeneratedTagSerializer:
    ...
    @pytest.mark.parametrize("color", ["#abc", "#abcd", "#aabbcc", "#aabbccdd"])
    def test_create_tag_with_valid_hex_color(self, color):
        user = User.objects.create(
            username=f"hexvalid_{color[1:]}@example.com",
            email=f"hexvalid_{color[1:]}@example.com",
        )
        c = APIClient()
        c.force_authenticate(user=user)
        resp = c.post(
            "/api/globals/tag/",
            {"field": "name", "value": "test", "values": {"color": color}},
            format="json",
        )
        assert resp.status_code == 201, resp.json()

    @pytest.mark.parametrize(
        "color", ["red", "#gg0000", "123456", "#12345", "#1234567"]
    )
    def test_create_tag_with_invalid_hex_color_returns_400(self, color):
        user = User.objects.create(
            username=f"hexinvalid_{color.replace('#', '')}@example.com",
            email=f"hexinvalid_{color.replace('#', '')}@example.com",
        )
        c = APIClient()
        c.force_authenticate(user=user)
        resp = c.post(
        ...
        assert "hex color" in resp.json()["detail"]
...
class TestAutoGeneratedClayBodySerializer:
    def test_short_description_included(self):
        ...
        user = User.objects.create(
            username="cbuser@example.com", email="cbuser@example.com"
        )
        cb = ClayBody.objects.create(
            user=user, name="Stoneware", short_description="High-fire gray"
        )
    ...
    def test_globals_endpoint_includes_short_description(self):
        user = User.objects.create(
            username="cbapi@example.com", email="cbapi@example.com"
        )
        ClayBody.objects.create(
            user=user, name="Porcelain", short_description="Translucent"
        )
        c = APIClient()
        c.force_authenticate(user=user)
...
class TestAutoGeneratedGlazeTypeSerializer:
    ...
    def test_globals_endpoint_includes_extra_fields(self):
        GlazeType.objects.create(
            user=None, name="Tenmoku", short_description="Iron black"
        )
        user = User.objects.create(
            username="gtapi@example.com", email="gtapi@example.com"
        )
        c = APIClient()
        c.force_authenticate(user=user)
...
class TestGlobalEntries:
    def test_global_entries_impl_delegates_to_shared_resolvers(self, monkeypatch):
        ...
        def fake_impl(
            ...
            *,
            ...
            seen["global_name"] = global_name
            seen["request"] = request
            seen["resolve_global"] = resolve_global
            seen["public_global"] = public_global
            seen["private_global"] = private_global
            return Response({"ok": True})

        monkeypatch.setattr(
            "api.global_entries.logic.global_entries_impl_with_resolvers", fake_impl
        )
        ...
        response = global_entries_impl(request, "location")

        assert response.data == {"ok": True}
        assert seen["global_name"] == "location"
        assert seen["resolve_global"].__name__ == "get_global_model_and_field"
    ...
    def test_default_get_serializer_used_when_no_rich_serializer_registered(
        self, client, monkeypatch
    ):
        monkeypatch.delitem(
            _GLOBAL_ENTRY_SERIALIZERS, apps.get_model("api", "Location"), raising=False
        )
    ...
    def test_default_post_response_used_when_no_rich_serializer_registered(
        self, client, monkeypatch
    ):
