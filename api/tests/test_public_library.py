...
class TestPublicLibraryGet:
    ...
    def test_non_public_global_only_shows_own_private_objects(
...
class TestPublicLibraryAdmin:
    ...
    def test_save_model_forces_user_to_none(self):
        ...
        ma.save_model(
            request, obj, type("FakeForm", (), {"cleaned_data": {}})(), change=False
        )
        obj.refresh_from_db()
    ...
    def test_get_form_uses_public_library_form_and_cloudinary_widget(self):
        ...
        superuser = User.objects.create_superuser(
            username="su-form@example.com",
            email="su-form@example.com",
            password="password",
        )
        ma = PublicLibraryAdmin(GlazeType, AdminSite())
    ...
    def test_glaze_combination_post_rejects_unknown_layer_pk(self, client):
        ...
        assert response.status_code == 400
        assert response.json() == {
            "detail": "Unknown GlazeType pk: '00000000-0000-0000-0000-000000000000'"
        }
...
class TestGlazeAdminSite:
    ...
    def test_index_without_public_models_does_not_add_public_libraries(
        ...
        monkeypatch.setattr(
            AdminSite, "get_app_list", lambda self, request, app_label=None: app_list
        )
