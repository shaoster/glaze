...
def test_get_state_summary_returns_declared_summary(monkeypatch):
    ...
    assert (
        workflow_module.get_state_summary("completed")
        == _MOCK_STATE_MAP["completed"]["summary"]
    )
...
def test_get_global_model_and_field_prefers_name(monkeypatch):
    ...
    model_cls, fields, display_field = workflow_module.get_global_model_and_field(
        "location"
    )
...
def test_get_image_fields_for_global_model_returns_empty_for_no_image_fields(
    monkeypatch,
):
    monkeypatch.setattr(workflow_module, "_GLOBALS_MAP", _MOCK_GLOBALS_MAP)
...
def test_get_taggable_globals_returns_declared_globals(monkeypatch):
    monkeypatch.setattr(workflow_module, "_GLOBALS_MAP", _MOCK_GLOBALS_MAP)
    monkeypatch.setattr(
        workflow_module, "get_global_names", lambda: list(_MOCK_GLOBALS_MAP.keys())
    )
