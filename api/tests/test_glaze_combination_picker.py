...
def _pub_combo(*glaze_types, **props) -> GlazeCombination:
    combo = GlazeCombination.get_or_create_with_components(
...
class TestGlazeCombinationFiltering:
    def test_filter_by_glaze_type_id(self, client):
        ...
        response = client.get(
            "/api/globals/glaze_combination/", {"glaze_type_ids": str(gt_a.pk)}
        )
    ...
    def test_filter_by_multiple_glaze_type_ids_requires_all(self, client):
        ...
        response = client.get(
            "/api/globals/glaze_combination/", {"glaze_type_ids": ids_param}
        )
    ...
    def test_filter_is_food_safe_true(self, client):
        ...
        response = client.get(
            "/api/globals/glaze_combination/", {"is_food_safe": "true"}
        )
    ...
    def test_filter_is_food_safe_false(self, client):
        ...
        response = client.get(
            "/api/globals/glaze_combination/", {"is_food_safe": "false"}
        )
    ...
    def test_no_filter_returns_all_visible(self, client, user):
        ...
        priv_combo = GlazeCombination.get_or_create_with_components(
            user=user, glaze_types=[gt2]
        )[0]
...
class TestFavoriteAdd:
    ...
    def test_idempotent(self, client, user):
        ...
        assert response.status_code == 204
        assert (
            FavoriteGlazeCombination.objects.filter(
                user=user, glaze_combination=combo
            ).count()
            == 1
        )
    ...
    def test_cannot_favorite_other_users_private_combination(
        ...
        response = client.post(
            f"/api/globals/glaze_combination/{priv_combo.pk}/favorite/"
        )
...
class TestFavoriteRemove:
    def test_removes_favorite(self, client, user):
        ...
        assert response.status_code == 204
        assert not FavoriteGlazeCombination.objects.filter(
            user=user, glaze_combination=combo
        ).exists()
