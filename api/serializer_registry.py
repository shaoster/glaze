...
Two dicts track the lifecycle:

- ``_GENERATED_GLOBAL_ENTRY_SERIALIZERS`` — auto-generated by
  ``make_global_entry_serializer()`` in serializer_factories.py, one entry per
  factory global.  These provide the default {id, <fields>, is_public} shape.

- ``_GLOBAL_ENTRY_SERIALIZERS`` — the combined runtime lookup used by views and
  logic.  Starts with the generated entries; hand-written serializers decorated
  with ``@global_entry_serializer`` overwrite their entry here so bespoke
  serializers always win without touching the generated dict.
"""

from django.db import models as django_models
from rest_framework import serializers

# Populated by make_global_entry_serializer() in serializer_factories.py.
_GENERATED_GLOBAL_ENTRY_SERIALIZERS: dict = {}

# Combined runtime lookup: generated entries overwritten by bespoke ones.
_GLOBAL_ENTRY_SERIALIZERS: dict = {}


def global_entry_serializer(model_cls: type[django_models.Model]):
    """Decorator that registers a hand-written serializer for the given global.
