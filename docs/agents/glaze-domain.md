...
- [`web/src/util/types.ts`](../../web/src/util/types.ts) derives domain types from `generated-types.ts` with explicit overrides for the fields the UI relies on. It also holds the `STATES` array and `SUCCESSORS` map from `workflow.yml`.
- If a frontend lint or type-check failure points at a missing field on a story or test fixture, treat it as an API-contract mismatch. Update the backend schema or the fixture to match the generated type, then regenerate types if needed. Do not relax the domain type to make the failure disappear.
