...
| `ALLOWED_HOSTS`                                    | `ALLOWED_HOST`                | `localhost`, `127.0.0.1`                    | Appends the single production hostname (e.g. `myapp.example.com`) and, in production, scopes the auth/CSRF cookies to the parent domain so `admin.<host>` shares the same session |
...
- [`web/src/util/types.ts`](../../web/src/util/types.ts) derives domain types from `generated-types.ts` with explicit overrides only for nested normalization (for example state/image/crop shapes). It also holds the `STATES` array and `SUCCESSORS` map from `workflow.yml`.
