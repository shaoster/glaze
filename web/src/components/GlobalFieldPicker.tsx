...
function buildDisplayOptions(entries: GlobalEntry[]): string[] {
  const privateNames = new Set(
    entries.filter((e) => !e.isPublic).map((e) => e.name),
  );
  return entries.map((e) =>
    e.isPublic && privateNames.has(e.name) ? e.name + PUBLIC_SUFFIX : e.name,
  );
}
...
export default function GlobalFieldPicker({
  ...
  const fetchError =
    optionsProp !== undefined
      ? null
      : fetchAsyncError
        ? `Failed to load ${label.toLowerCase()} options. Please refresh.`
        : null;
  ...
  const displayOptions = useMemo(
    () => buildDisplayOptions(sortedEntries),
    [sortedEntries],
  );
  ...
  const isFavorite = selectedEntry
    ? getEffectiveFavorite(selectedEntry)
    : false;
