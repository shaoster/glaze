...
describe("GlobalFieldPicker", () => {
    ...
    it("shows helperText when provided and there is no error", async () => {
      await act(async () => {
        render(
          <GlobalFieldPicker
            {...defaultProps}
            helperText="Where is this piece stored?"
          />,
        );
      });
      expect(
        screen.getByText("Where is this piece stored?"),
      ).toBeInTheDocument();
    });
  ...
  describe("options — internal fetch", () => {
    ...
    it("shows an error message when fetchGlobalEntries fails", async () => {
      vi.mocked(api.fetchGlobalEntries).mockRejectedValue(
    ...
    it("shows fetched options in the dropdown", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByRole("option", { name: "Studio A" }),
        ).toBeInTheDocument();
        expect(
          screen.getByRole("option", { name: "Studio B" }),
        ).toBeInTheDocument();
      });
    });
    ...
    it("shows provided options in the dropdown", async () => {
      render(
        <GlobalFieldPicker
          {...defaultProps}
          options={[entry("Shelf 1"), entry("Shelf 2")]}
        />,
      );
      await userEvent.click(screen.getByLabelText("Location"));
      expect(
        screen.getByRole("option", { name: "Shelf 1" }),
      ).toBeInTheDocument();
      expect(
        screen.getByRole("option", { name: "Shelf 2" }),
      ).toBeInTheDocument();
    });
  });
  ...
  describe("creating a new entry", () => {
    ...
    it("shows an error message when createGlobalEntry fails", async () => {
      vi.mocked(api.createGlobalEntry).mockRejectedValue(
        new Error("Network error"),
      );
      render(<Controlled canCreate />);
      ...
      await waitFor(() =>
        expect(
          screen.getByRole("option", { name: 'Create "Bad Studio"' }),
        ).toBeInTheDocument(),
      );
      fireEvent.click(
        screen.getByRole("option", { name: 'Create "Bad Studio"' }),
      ...
      await waitFor(() =>
        expect(
          screen.getByText("Failed to create location. Please try again."),
        ).toBeInTheDocument(),
      );
    });
    ...
    it("hides helperText and shows error when creation fails", async () => {
      ...
      await waitFor(() =>
        expect(
          screen.getByRole("option", { name: 'Create "X"' }),
        ).toBeInTheDocument(),
      );
  ...
  describe("uncommitted text", () => {
    it("does not call onChange while the user is typing", async () => {
      const onChange = vi.fn();
      render(
        <GlobalFieldPicker {...defaultProps} onChange={onChange} canCreate />,
      );
      await userEvent.type(screen.getByLabelText("Location"), "New Studio");
  ...
  describe("public/private disambiguation", () => {
    it("appends (public) suffix to a public entry that shares a name with a private entry", async () => {
      ...
      await userEvent.click(screen.getByLabelText("Location"));
      expect(
        screen.getByRole("option", { name: "Stoneware" }),
      ).toBeInTheDocument();
      expect(
        screen.getByRole("option", { name: "Stoneware (public)" }),
      ).toBeInTheDocument();
    });
    ...
    it("does not append (public) suffix when there is no name conflict", async () => {
      ...
      expect(
        screen.queryByRole("option", { name: "Stoneware (public)" }),
      ).not.toBeInTheDocument();
    });
  ...
  describe("favorites (favoritable globals)", () => {
    ...
    it("favorites appear first in the options list", async () => {
      ...
      await waitFor(() => {
        ...
        const ironRedIdx = options.findIndex(
          (o) => o.textContent === "Iron Red",
        );
        const celadonIdx = options.findIndex(
          (o) => o.textContent === "Celadon",
        );
    ...
    it("newly created entries are not auto-favorited", async () => {
      ...
      await userEvent.type(
        screen.getByLabelText("Glaze Combination"),
        "New Glaze",
      );
      await waitFor(() =>
        expect(
          screen.getByRole("option", { name: 'Create "New Glaze"' }),
        ).toBeInTheDocument(),
      );
      fireEvent.click(
        screen.getByRole("option", { name: 'Create "New Glaze"' }),
      ...
      await waitFor(() =>
        expect(screen.getByLabelText("Glaze Combination")).toHaveValue(
          "New Glaze",
        ),
      );
