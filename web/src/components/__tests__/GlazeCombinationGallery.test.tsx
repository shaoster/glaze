...
describe("GlazeCombinationGallery", () => {
  ...
  describe("with data", () => {
    ...
    it("opens a lightbox with a Go to the Piece button when a piece image is clicked", async () => {
      ...
      expect(
        screen.getByRole("button", { name: /Go to the Piece/i }),
      ).toBeInTheDocument();
    });
  ...
  describe("with a test tile image", () => {
    ...
    it("opens a lightbox without a Go to Piece button when the test tile is clicked", async () => {
      ...
      renderGallery();
      await waitFor(() =>
        screen.getByRole("button", { name: /View test tile/i }),
      );
      await userEvent.click(
        screen.getByRole("button", { name: /View test tile/i }),
      );
      ...
      expect(
        screen.queryByRole("button", { name: /Go to the Piece/i }),
    ...
    it("renders thumbnails for each image across all pieces", async () => {
      ...
      await waitFor(() => {
        expect(
          screen
            .getAllByRole("img")
            .filter((img) => img.getAttribute("src")?.includes("example.com")),
